//! An input component whose values are bounded to a min and max
//! Follows
//! https://tms-dev-blog.com/webassembly-rust-front-end-with-yew-how-to-p1/

use web_sys::HtmlInputElement;
use yew::{
    html, Component, Html, InputEvent, MouseEvent, NodeRef, Properties,
    TargetCast,
};
    ...
    fn view(&self, ctx: &yew::Context<Self>) -> Html {
        ...
        let on_click = {
            ...
            ctx.link().callback(move |_e: MouseEvent| {
                let value_input_element =
                    cur_input.cast::<HtmlInputElement>().unwrap();
        ...
        let display_values =
            self.value_list.iter().map(|value| html! {<li>{value}</li>});
        html! {
            <div>
                <div>
                    <label>
                        {format!("Input value. Min: {}, Max: {}", min_val, max_val)}
                    </label>
                    <br />
                    <input ref={self.input_ref.clone()} placeholder="input a number"
                        oninput={on_input}
                        min={min_val.to_string()} max={max_val.to_string()}
                        class="w-64 rounded bg-neutral-900 border \
                               border-neutral-700"/>
    ...
    fn update(
        ...
        match msg {
            Msg::SetInput(val) => {
                ...
                let input_element =
                    self.input_ref.clone().cast::<HtmlInputElement>().unwrap();
