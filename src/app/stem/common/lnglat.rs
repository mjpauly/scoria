...
impl std::str::FromStr for LngLat {
    ...
    /// Parse a *lat,lng* coordinate string in the format of "35.0, -94.0".
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let nowhitespace = s.replace(char::is_whitespace, "");
        let mut split = nowhitespace.split_terminator(',');
        let lat_str = split.next().ok_or("No lat")?;
        let lng_str = split.next().ok_or("No lng")?;
        let lat = lat_str.parse::<f64>().map_err(|_| "Bad lat")?;
        let lng = lng_str.parse::<f64>().map_err(|_| "Bad lng")?;
        Ok(LngLat { lng, lat })
    }
}
