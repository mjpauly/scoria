...
static SCORIA_1_5_0: &[u8] = include_bytes!(env!("SCORIA_1_5_0"));
static SCORIA_1_4_4: &[u8] = include_bytes!(env!("SCORIA_1_4_4"));
...
pub fn get_apk_file_services() -> impl HttpServiceFactory {
    web::scope("/download/apk")
        ...
        .service(web::redirect("/latest", "./Scoria_1.5.0.apk"))
        .service(latest_version)
        // NOTE: on updates, add new route for the new verison here:
        .service(scoria_1_5_0)
        .service(scoria_1_4_4)
...
async fn latest_version() -> impl Responder {
    HttpResponse::Ok().body("8,1.5.0") // NOTE: update this on updates
}

#[get("/Scoria_1.5.0.apk")]
async fn scoria_1_5_0() -> impl Responder {
    tracing::info!("Download of 1.5.0");
    HttpResponse::Ok()
        .insert_header(APK_CONTENT_TYPE_HEADER)
        .body(SCORIA_1_5_0)
}

#[get("/Scoria_1.4.4.apk")]
async fn scoria_1_4_4() -> impl Responder {
    tracing::info!("Download of 1.4.4");
    HttpResponse::Ok()
        .insert_header(APK_CONTENT_TYPE_HEADER)
        .body(SCORIA_1_4_4)
}
